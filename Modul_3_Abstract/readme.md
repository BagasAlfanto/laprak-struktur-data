...
Modul 3 membahas Abstract Data Type (ADT), yakni konsep yang memisahkan apa yang bisa dilakukan terhadap data dari bagaimana cara melakukannya. Dengan ADT, kita dapat memanggil operasi seperti tampil(), tukar(), atau input() tanpa perlu memahami detail implementasinya.

ADT biasanya diwujudkan lewat struct, class, atau modul, yang menyembunyikan struktur internal (mis. array atau pointer) dari pengguna. Pengguna hanya berinteraksi melalui antarmuka operasi yang disediakan — prinsip ini dikenal sebagai enkapsulasi. Intinya ADT menggabungkan data dan operasi untuk memanipulasinya, sementara implementasinya tetap tersembunyi agar antarmukanya konsisten.

Pada mata kuliah struktur data, ADT seperti Stack dan Queue penting dipahami karena sifatnya tidak bergantung pada implementasi; baik diimplementasikan dengan array maupun linked list, perilaku dari sisi pengguna sama.
